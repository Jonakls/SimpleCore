...
public class SurvivalCommand implements CommandExecutor {
    ...
    public SurvivalCommand(Service service){
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!(args.length > 0)){
            ...
            p.sendMessage(service.getFiles().getLang().getString("gamemode.change").replace("%type%", service.getFiles().getLang().getString("gamemode.type.survival")));
            return true;
        }
        ...
        target.sendMessage(service.getFiles().getLang().getString("gamemode.target-change").replace("%type%", service.getFiles().getLang().getString("gamemode.type.survival")).replace("%player%", p.getName()));
